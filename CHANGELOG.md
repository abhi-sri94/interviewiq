...
* **Scroll-Snap Carousels**: Replaced vertical card stacks for Features & Steps sections with touch-friendly CSS horizontal carousels (`snap-x snap-mandatory flex`) in `HomePage.jsx`.
* **Tablet Scaling**: Deferred the 3-column desktop grid transition from `md` (768px) to `lg` (1024px) in `HomePage.jsx`. On tablets, cards scale to `sm:w-[48%]` and display as a spacious horizontal carousel.
...
* **Homepage Logo Redirects**: Changed all logo click paths from the conditional `/dashboard` paths to direct home links (`/`) in:
  * `Navbar.jsx` (Public header logo).
  * `DashboardLayout.jsx` (Mobile header logo & desktop sidebar logo).

---

## ✏️ 3. Global Typography Sizing
* **Root Font Scale**: Adjusted global relative text scaling inside `index.css`:
...
* **Metadata Insertion**: Configured sharing previews in `index.html`:
...
* **Rich Preview Thumbnail**: Generated a custom high-end dashboard mockup preview image, saved as `public/og-image.png`, which is automatically referenced as the OG share thumbnail.
...
* **Standalone Support**: Added W3C compliant `manifest.json` in the `public` directory, enabling standalone application shells.
...
* **Speech Synthesis**: Created a reactive voice generator hook in `InterviewPage.jsx` that automatically speaks verbal mock questions aloud using the Web Speech Synthesis API.
...
* **Mic Waveform Visualizer**: Configured bouncing equalizer keyframes in `index.css`. When recording is active (`isListening === true`), the Question Card displays a pulsing equalizer visualizer with 5 bouncing red bars.
* **Monaco Custom Controls**: Added state dropdown controls next to the editor language selector in `InterviewPage.jsx`:
